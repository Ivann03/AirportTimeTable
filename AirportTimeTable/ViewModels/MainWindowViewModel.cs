...
namespace AirportTimeTable.ViewModels
{
    public class MainWindowViewModel : ViewModelBase
    {
        ...
        public void CurentColectionUpdate(int typeFirst, int typeDate)
        {
            ...
            if (typeFirst == 1) { curentNaznach = "Вылетел"; }
            else { curentNaznach = "Рейс прибыл"; }
            DateTime curTime = DateTime.Now;
            var needDay = 0;
            if (typeDate == 1) needDay = curTime.Day - 1;
            else if (typeDate == 2) needDay = curTime.Day;
            else needDay = curTime.Day + 1;
